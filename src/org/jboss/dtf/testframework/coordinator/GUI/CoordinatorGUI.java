...
public class CoordinatorGUI extends JFrame implements ActionListener
{
	private HashMap _supportedOS = new HashMap();
	private HashMap _supportedProducts = new HashMap();
		...
		if ( obj instanceof CheckableTreeNode )
		{
			...
			switch (checkTreeNode.getType())
			{
				case CheckableTreeNode.TYPE_OS :
					...
					System.out.println(checkTreeNode.getOSName());
				...
				case CheckableTreeNode.TYPE_PRODUCT :
					...
					System.out.println(checkTreeNode.getProductName());
				...
				case CheckableTreeNode.TYPE_GROUP :
					...
					System.out.println(checkTreeNode.getGroupName());
				...
				case CheckableTreeNode.TYPE_TEST :
					...
					System.out.println(checkTreeNode.getTestName());
		...
		for (int count=0;count<osName.length;count++)
		{
			...
			for (int productCount=0;productCount<productName.length;productCount++)
			{
				...
				for (int testGroupCount=0;testGroupCount<testGroups.length;testGroupCount++)
				{
                    ...
                    HashMap subtree = testGroup.getSubTree();

                    Object[] subGroups = subtree.keySet().toArray();
	...
	private void retrieveConfiguration(String testDefinitionsURL)
	{
		try
		{
			 ...
			 * by the test system
			 */
			List productConfigs = supportedProductElement.getChildren("products");
			...
			for (int count=0;count<productConfigs.size();count++)
			{
				...
				_supportedProducts.put(displayName, name);
			}
			...
			for (int count=0;count<osConfigs.size();count++)
			{
				...
				Object[] supportedProductList = osConfig.getChildren().toArray();

				for (int childCount=0;childCount<supportedProductList.length;childCount++)
				{
					Element supportedProducts = (Element)supportedProductList[childCount];
					String productName = supportedProducts.getAttributeValue("name");
            ...
            _testGroups = getTestGroups(testDefRoot);
        }
        catch (Exception e)
		{
			e.printStackTrace();
    ...
    private HashMap getTestGroups(Element root) throws Exception
    {
        ...
        for (int count=0;count<testGroups.size();count++)
        {
            ...
            groups.put(groupName, new TestGroup(groupName,testsInGroup, getTestGroups(testGroup)));
        }
